...
class GUI:
    ...
    def train(self, iters=5000):
        if iters > 0:
            for i in tqdm.trange(iters):
                if self.deform.name == 'node' and self.iteration_node_rendering < self.opt.iterations_node_rendering:
                    # first pre-train sparse control points and deformation MLP, where treat control points as gaussians
                    self.train_node_rendering_step()
                else:
    ...
    def train_node_rendering_step(self):
        ...
        loss.backward()
        with torch.no_grad():
            # node and MLP pre-training procedure
            # 1. before downsampling nodes: do densification, pruning and resetting
            # 2. do downsampling
            # 3. optimize nodes and MLP as 3D gaussians
            # 4. utilize gaussians' parameters to initialize nodes
            if self.iteration_node_rendering < self.opt.iterations_node_sampling:
                # Densification like gaussians instead of adaptive node control 
